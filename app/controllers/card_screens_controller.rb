class CardScreensController < ApplicationController
  def ecran1
    ...
    # @choice_categories_select = @choice_categories_with_votes.sort_by { |choice_category| choice_category.rank }[0..4]
  ...
  def list_games(choice_array)
    @result_category = choice_array.sort_by { |choice_category| choice_category.rank }.last(2)
    @result1 = @result_category.map do |result|
      if @room.price_filter == "Gratuit"
        if @room.player_number == 1
          result.category.games.where(price: nil, solo:"Solo").map { |game| game }
        else
          result.category.games.where(price: nil, multi:"Multijoueur").map { |game| game }
        end
      else
        if @room.player_number == 1
        result.category.games.where(solo:"Solo").map { |game| game }
        else
          result.category.games.where(multi:"Multijoueur").map { |game| game }
        end
      end
    end
